...
def registration():
    return render_template("ureg.html",msg='Successfully Registereted')
...
def verify():
    if request.method=='POST':
        ...
        if (pwd == cpass):
            sql = 'INSERT INTO registration (name,email,pwd) VALUES (%s, %s, %s)'
            val = (name,email,pwd)
            mycursor.execute(sql,val)
            mydb.commit()
            return render_template('ureg.html',msg='Successfully Registrated')
...
def userlog():
    if request.method =='POST':
        ...
        cursor = mydb.cursor()
        sql = "select * from registration where email='%s' and pwd='%s'" % (username,password1)
        print('q')
        ...
        if len(results) > 0:
            print("r")
            flash("welcome to website","sussess")
            return render_template("userhome.html",msg=results[0][1])
        else:
            flash("Invaild Email/password",'danger')
